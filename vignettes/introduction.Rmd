...
Step 1: Install SCOPEr1 package from github (This needs some tweaking)
```{r}
devtools::install_github("kacebron/SCOPEr")
```

Step 2: Initiate run_scope to run a test verifying integrity of copy of SCOPE model. This will initially prompt you to manually choose the location of SCOPE.m file. Once located, the filepath is saved in your global environment and is only deleted once the session is ended.
```{r}
run_scope()
```
...
Step 5: Create a variable parameter for time series input and save it as par_variable object. 
```{r}
