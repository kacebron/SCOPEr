...
migrate_file <- function() {
  ...
  file.copy(from = orig_input_data_default, to = new_input_data_default, overwrite = TRUE)
  file.copy(from = orig_input_data_latin_hypercube, to = new_input_data_latin_hypercube, overwrite = TRUE)
  ...
  if (file.exists(new_set_paramFile) &&
    ...
    file_mod_time_paramFile <- file.info(new_set_paramFile)$mtime
    file_mod_time_setoptions <- file.info(new_setoptions)$mtime
    file_mod_time_filenames <- file.info(new_filenames)$mtime
    file_mod_time_input_data_default <- file.info(new_input_data_default)$mtime
    ...
    if (difftime(current_time, file_mod_time_paramFile, units = "secs") < time_threshold &&
        difftime(current_time, file_mod_time_setoptions, units = "secs") < time_threshold &&
