...
#' @param dataset_dir Lets you specify the directory of the dataset containing the input files for time series simulations
#' @param meteo_ec_csv Lets you specify the name of the .csv file that contains the meteorological information to be used in time series runs (more of this in the create_ts_input() function)
#' @param tts Solar zenith angle
#' @param RH relative humidity
#' @param VPD vapor pressure deficit
#' @param tto Observer zenith angle
#' @param psi Relative azimuth angle: the azimuthal difference between solar and observation angle.
...
#' @param Car Carotenoid content. Usually 25% of Cab if options.Cca_function_of_Cab (ug cm-2)
...
set_filenames <- function(simulation_name,
  ...
  filenames <- stringr::str_replace_all(filenames,
                               c("(\\n)$" = "",
                                 ...
                                 "(?<=BSMBrightness,)((BSMBrightness){1,}|)" = ifelse(BSMBrightness == TRUE, "BSMBrightness", ""),
